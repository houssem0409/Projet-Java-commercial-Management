...
public class DetailsBonLivraison {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DetailsBonLivraison window = new DetailsBonLivraison();
	 ...
	 */
	public DetailsBonLivraison() throws ClassNotFoundException, SQLException {
		DataBaseConnection.connecter();
		initialize();
		
		BonLivraisonFrame.tfidbon.getText();
		int id=Integer.parseInt(BonLivraisonFrame.tfidbon.getText());
		detailsbons(id);
		detailbon2(id);
		Total(id);
	}
	...
	private void initialize() {
		...
		lblNewLabel = new JLabel("Date Du Bon Livraison :");
	...
	public void detailsbons(int id) {
		String req= "SELECT reference,designation,prix,uniteMesure,tva,quantite FROM `lignebonlivraison`,`produit` where lignebonlivraison.idBonLivraison="+id+" and lignebonlivraison.idProduit=produit.id_produit  ";
		ResultSet rs = null;
		try {
	...
	public void detailbon2(int id) {
		...
		try {
			...
			String req= "SELECT dateBon,dateLivraison,adresseBon,numCommande FROM `bonlivraison` where id_bonlivraison="+id+" ";
	...
	public void Total(int id) throws SQLException {
		String req= "SELECT prix,quantite FROM `lignebonlivraison`,`produit` where lignebonlivraison.idBonLivraison="+id+" and lignebonlivraison.idProduit=produit.id_produit  ";
		ResultSet rs = null;
		try {
			rs = DataBaseConnection.executionQuery(req);
		} catch (SQLException e1) {
